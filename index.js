...
//   // })
//   //
//   // let startSplicedAr = strAr.splice(startSplice, strAr.length -1)
//   //
//   // startSplicedAr.forEach((el, idx)=>{
//   //   if (el == ".") {
//   //     endSplice = idx
//   //   }
//   // })
//   //
//   //
//   // return startSplicedAr.splice(0, endSplice).join("")
//
...
//     }
//   }
//   if (dotIdxAr.length>1) {
//     // strAr.splice(dotIdxAr[0], dotIdxAr[1]).join("")
//     console.log(dotIdxAr);
//   }
//   return strAr.slice(dotIdxAr[0] + 1,dotIdxAr[1])
// }


function domainName(url){
  url = url.replace("https://", '');
  url = url.replace("httpn://", '');
  url = url.replace("www.", '');
  return url.split(".")[0]
}


console.log(domainName("httpn://www.google.com"));
...
function filterScripts(scriptAr) {
  let passed = []
  scriptAr.forEach((el)=>{
    if (el.living) {
      passed.push(el)
...
function scriptMap(scriptAr) {
  return scriptAr.filter(el => el)
}
...
return null;
 ...
 function getRange(code) {
  ...
  SCRIPTS.forEach((script)=>{
    script.ranges.forEach((el)=>{
      if (code >= el[0] && code <=el[1]) {
        answer = script
      }
    })
  })
...
function speak() {
  console.log("speak test on", `${this.name}`);
}

// let rabbitOne = {name: "vibzy", speak}
let rabbitTwo = {name: "dhruv g", speak}
...
let rabbitOne = Object.create(protoRabbit)

rabbitOne.name = "vibzyyyy"
